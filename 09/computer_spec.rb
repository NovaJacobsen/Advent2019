...
module Day09
  describe Computer do
    describe 'Opcodes' do
      ...
      it 'can increase the relative point with 9' do
        computer = Computer.new(
          109, 5,
          109, -2,
          99
        ...
        computer.run
        expect(computer.relative).to eq 3
      end
    end

    describe 'read modes' do
      ...
      it 'can read in relative mode' do
        computer = Computer.new(
          204, -4,
          99,
        )
        computer.relative = 6
        computer.run
        expect(computer.output).to eq [99]
      end

      it 'can write in relative mode' do
        computer = Computer.new(
          203, -5,
          4, 1,
          99,
        ...
        computer.relative = 6
        computer.run
        expect(computer.output).to eq [95]
      end
    end
    ...
    it('can read and write beyond initial program size') do
      computer = Computer.new(
        1001, 15, 5, 15,
        4, 15,
        99
      ...
      computer.run
      expect(computer.output).to eq [5]
    end
